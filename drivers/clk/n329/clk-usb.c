...
#define to_clk_usb(_hw) container_of(_hw, struct clk_usb, hw)

static int clk_usb_enable(struct clk_hw *hw)
{
	struct clk_usb *usb_clk = to_clk_usb(hw);
	u32 reg;
...
unsigned long clk_usb_best_rate(unsigned long rate, unsigned long *ret_pll_div,
...
static unsigned long clk_usb_recalc_rate(struct clk_hw *hw,
		unsigned long parent_rate)
{
	if (!clk_usb_is_enabled(hw))
		return 0;

	return parent_rate;
}

static long clk_usb_round_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long *prate)
{
	...
	if (!clk_usb_is_enabled(hw))
		...
		best_rate = clk_usb_best_rate(rate, NULL, NULL);

	return best_rate;
}

static int clk_usb_set_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
	...
	best_rate = clk_usb_best_rate(rate, &best_pll_div, &best_src);

	if (best_src == 3) {
		clk_set_parent(n329_clocks_get(usb_uclk),
				n329_clocks_get(udiv0_clk + best_pll_div));
		clk_set_parent(n329_clocks_get(usb_src), 
				n329_clocks_get(usb_uclk));
	} else if (best_src == 2) {
		clk_set_parent(n329_clocks_get(usb_aclk),
				n329_clocks_get(adiv0_clk + best_pll_div));
		clk_set_parent(n329_clocks_get(usb_src), 
				n329_clocks_get(usb_aclk));
	} else {
		clk_set_parent(n329_clocks_get(usb_src), 
				n329_clocks_get(xtal_clk));
	}
	clk_set_rate(n329_clocks_get(usb_div), best_rate);

	return 0;
}

const struct clk_ops clk_usb_ops = {
	.enable = clk_usb_enable,
	.disable = clk_usb_disable,
	.is_enabled = clk_usb_is_enabled,
	.recalc_rate = clk_usb_recalc_rate,
	.round_rate = clk_usb_round_rate,
	.set_rate = clk_usb_set_rate,
};
EXPORT_SYMBOL_GPL(clk_usb_ops);
...
struct clk *clk_register_usb(struct device *dev, const char *name,
		...
		spinlock_t *lock)
{
	struct clk_usb *usb_clk;
	struct clk *clk;
	struct clk_init_data init;

	/* allocate the usb_clk */
	usb_clk = kzalloc(sizeof(struct clk_usb), GFP_KERNEL);
	if (!usb_clk) {
...
EXPORT_SYMBOL_GPL(clk_register_usb);
